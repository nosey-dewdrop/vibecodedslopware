#!/bin/zsh
# olc-gorunum.sh <cikti.json> — kurulu siteyi acar, gorunum.js kosturur.
# CSS degistirmeden ONCE ve SONRA kosulur; iki json diff'lenir.
set -e
KOK=${0:a:h}
HEDEF=${1:?kullanim: ./olc-gorunum.sh cikti.json}
...
"$CH" --headless=new --disable-gpu --remote-debugging-port=9223 \
      ...
      "http://localhost:8902/slopware/localhost/" >/dev/null 2>&1 &
sleep 5
